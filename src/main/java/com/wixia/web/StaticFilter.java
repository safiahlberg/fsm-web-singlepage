...
public class StaticFilter implements Filter {
    ...
    public static final String EXCLUDED_MIME_TYPES = "excluded-mime-types";
    ...
    private void doBeforeProcessing(RequestWrapper request, ResponseWrapper response)
        ...
        if (debug) {
            log("StaticFilter:DoBeforeProcessing");
        }
    ...
    private void doAfterProcessing(RequestWrapper request, ResponseWrapper response)
            throws IOException, ServletException {
        if (debug) {
            log("StaticFilter:DoAfterProcessing");
        }

        if (filterConfig.getInitParameter(EXCLUDED_MIME_TYPES) != null && response.getContentType() != null) {
            final Collection<String> excludedMimeTypes = Arrays.asList(filterConfig.getInitParameter(EXCLUDED_MIME_TYPES).trim().split(REGEX));
            final Collection<String> contentTypes = Arrays.asList(response.getContentType().trim().split(REGEX));
            
            if (!CollectionUtils.containsAny(contentTypes, excludedMimeTypes)) {
                final Calendar cal = GregorianCalendar.getInstance();
                cal.add(Calendar.MONTH, 6);
                response.setDateHeader(EXPIRES, cal.getTimeInMillis());
                response.setHeader(CACHE_CONTROL, "public, max-age=" + 60 * 60 * 24 * 7);
            }
        }
    ...
    public void init(FilterConfig filterConfig) {
        ...
        if (filterConfig != null) {
            if (debug) {
                log("StaticFilter: Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        ...
        StringBuffer sb = new StringBuffer("StaticFilter(");
    ...
    class RequestWrapper extends HttpServletRequestWrapper {
        ...
        public void setParameter(String name, String[] values) {
            if (debug) {
                System.out.println("StaticFilter::setParameter(" + name + "=" + values + ")" + " localParams = " + localParams);
            }
        ...
        public String getParameter(String name) {
            if (debug) {
                System.out.println("StaticFilter::getParameter(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public String[] getParameterValues(String name) {
            if (debug) {
                System.out.println("StaticFilter::getParameterValues(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Enumeration getParameterNames() {
            if (debug) {
                System.out.println("StaticFilter::getParameterNames() localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Map getParameterMap() {
            if (debug) {
                System.out.println("StaticFilter::getParameterMap() localParams = " + localParams);
            }
            if (localParams == null) {
